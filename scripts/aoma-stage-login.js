...
(async () => {
  ...
  while (Date.now() < deadline) {
    ...
    if (!usernameFilled) {
      try {
        ...
        if (await userField.isVisible({ timeout: 500 }).catch(() => false)) {
          ...
          await userField.fill(AAD_USERNAME).catch(async () => {
            await userField.type(AAD_USERNAME, { delay: 15 });
          });
          await page
    ...
    if (!passwordFilled) {
      try {
        const pwField = page
          .locator(
            'input[type="password"][placeholder="Password" i], input[type="password"][aria-label*="password" i], input[name="passwd"], input[name="pwd"], input[type="password"]'
          )
          .first();
        if (await pwField.isVisible({ timeout: 3000 }).catch(() => false)) {
          console.log("Filling password field...");
          await pwField.fill(AAD_PASSWORD).catch(async () => {
            await pwField.type(AAD_PASSWORD, { delay: 20 });
          });
          console.log("Clicking Sign in button...");
          await page
            .locator(
              'input[type="submit"][value*="Sign in" i], button[type="submit"]:has-text("Sign in"), input[type="submit"], button[type="submit"]'
            )
            .first()
            .click()
            .catch(() => {});
    ...
    if (currentHost === stageHost) {
      ...
      if (currentUrl.includes("chain=Login")) {
        ...
        if (onLoginPage && checkCount < 5) {
          ...
          await page
            .goto(`${STAGE_URL}/`, { waitUntil: "networkidle", timeout: 30000 })
            .catch(() => {});
          await page.waitForTimeout(3000);
          continue;
        }
  ...
  const atStage = (() => {
    try {
      return new URL(currentUrl).host === stageHost;
    } catch {
      return false;
    }
  })();
