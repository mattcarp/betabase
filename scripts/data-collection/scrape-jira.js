...
} = require("../../utils/embeddings/openai");
const {
...
const maxTickets = args.includes("--limit") ? parseInt(args[args.indexOf("--limit") + 1]) : 1000;
...
async function extractTicketsFromPage(page) {
  return await page.evaluate(() => {
    ...
    const issueRows = document.querySelectorAll(
      '[data-issue-key], .issue-row, [data-testid="issue.views.issue-base.foundation.summary.heading"]'
    );
  ...
  try {
    ...
    const details = await page.evaluate(() => {
      ...
      const descriptionElement = document.querySelector(
        '[data-testid="issue.views.field.rich-text.description"], .user-content-block, .description'
      );
