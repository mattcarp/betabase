...
async function runPerformanceTests() {
  ...
  for (const query of TEST_QUERIES) {
    ...
    const standardTime = await measurePerformance(
      "  Standard (IVFFlat)",
    ...
    const fastTime = await measurePerformance(
      "  Optimized Fast (HNSW)",
    ...
    // Test smart search
    const smartTime = await measurePerformance(
      "  Smart Search (Auto)",
  ...
  const batchResults = await optimizedService.batchSearch(
    TEST_QUERIES.slice(0, 5),
    { matchCount: 5, mode: "fast" }
  );
