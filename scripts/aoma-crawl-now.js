  ...
  for (const pagePath of AOMA_PAGES) {
    try {
      ...
      await page.evaluate(() => {
        ...
        const expandables = document.querySelectorAll('[aria-expanded="false"], .mat-expansion-panel, .mat-tab-label');
      ...
      // Save HTML
      await fs.writeFile(
        `${outputDir}/${safeFileName}.html`,
        pageContent.html
      );
      
      // Save text content
      await fs.writeFile(
        `${outputDir}/${safeFileName}.txt`,
        pageContent.text
      );
