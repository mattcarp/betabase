...
async function main() {
  ...
  metrics.push(
    await measureLatency("Network RTT", async () => {
  ...
  metrics.push(
    await measureLatency("RPC Overhead", async () => {
  ...
  metrics.push(
    await measureLatency("AOMA Knowledge Query", async () => {
      const response = await fetch(`${AOMA_URL}/rpc`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  console.log(
    `   ${metrics[metrics.length - 1].status === "fail" ? "✗" : "✓"} ${metrics[metrics.length - 1].duration.toFixed(0)}ms`
  );

  // Test 4: Vector Search Performance
  console.log("\n4️⃣  Testing vector search (Jira)...");
  metrics.push(
    await measureLatency("Jira Search", async () => {
      const response = await fetch(`${AOMA_URL}/rpc`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
