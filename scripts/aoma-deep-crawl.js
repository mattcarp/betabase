...
async function extractMaximalContent(page) {
  ...
  await page.evaluate(() => {
    ...
    document.querySelectorAll('[aria-expanded="false"], .collapsible, .accordion, .expand-button').forEach(el => {
      el.click();
    });
  ...
  return await page.evaluate(() => {
    ...
    const content = {
      ...
      forms: Array.from(document.querySelectorAll('form')).map(form => ({
        ...
        fields: Array.from(form.querySelectorAll('input, select, textarea')).map(field => ({
          ...
          options: field.tagName === 'SELECT' ? Array.from(field.options).map(opt => opt.text) : []
        }))
      })),
    ...
    content.isMeaningful = content.text.length > 500 && 
                          !content.text.includes('Employee Login') ||
...
async function deepCrawl() {
  ...
  await page.goto('https://aoma-stage.smcdp-de.net/servlet/com.sonymusic.aoma.AOMADispatcherServlet?chain=CustomHomePageViewAction');
  await page.waitForTimeout(3000);
