...
async function main() {
  ...
  for (const query of queries) {
    ...
    if (vectorResult.data.data) {
      ...
      const avgScore =
        vectorResult.data.data.reduce((sum, r) => sum + r.score, 0) / vectorResult.data.data.length;
      console.log(`\n   📊 Average Relevance Score: ${avgScore.toFixed(4)}`);
      console.log(`   📊 Top Result Score: ${vectorResult.data.data[0]?.score.toFixed(4)}`);
      console.log(
        `   📊 Score Range: ${vectorResult.data.data[vectorResult.data.data.length - 1]?.score.toFixed(4)} - ${vectorResult.data.data[0]?.score.toFixed(4)}`
      );
    }
    ...
    if (assistantResult.answer) {
      console.log(
        `   ⏱️  Time: ${assistantResult.duration.toFixed(0)}ms (${(assistantResult.duration / 1000).toFixed(1)}s)`
      );
