...
async function loginAndCrawl() {
  ...
  try {
    ...
    const employeeLoginButton = await page
      .locator('#aadLoginBtn, a:has-text("Employee Login")')
      .first();
    ...
    await page
      .waitForURL(/login\.microsoftonline\.com|login\.live\.com|aoma.*login/i, { timeout: 10000 })
      .catch(() => {});
    ...
    if (currentUrl.includes("microsoftonline") || currentUrl.includes("login.live")) {
      ...
      const emailInput = await page
        .locator('input[type="email"], input[name="loginfmt"], #i0116')
        .first();
    ...
    await page
      .waitForURL(/aoma-stage\.smcdp-de\.net(?!.*login)/i, { timeout: 30000 })
      .catch(() => {});
