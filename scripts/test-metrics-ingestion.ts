...
async function testMetricsIngestion() {
  ...
  try {
    ...
    for (const metric of customMetrics) {
      const vectorId = await metricsService.recordCustomMetric(
        metric.name,
        metric.value,
        {
      ...
      console.log(`  ✅ Recorded: ${metric.name} = ${metric.value}${metric.unit ? ' ' + metric.unit : ''}`);
    }
    ...
    for (const query of searchQueries) {
      ...
      results.slice(0, 3).forEach((result, index) => {
        ...
        console.log(`       Value: ${result.metadata?.value}${result.metadata?.unit ? ' ' + result.metadata.unit : ''}`);
    ...
    const typeBreakdown = history.reduce((acc, metric) => {
    ...
    const verificationResults = await vectorService.searchVectors(
      'api response time',
      {
        matchThreshold: 0.7,
        matchCount: 5,
    ...
    if (verificationResults.length > 0) {
      ...
      console.log(`    Value: ${sample.metadata?.value}${sample.metadata?.unit ? ' ' + sample.metadata.unit : ''}`);
