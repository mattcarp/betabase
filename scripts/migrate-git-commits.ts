...
async function main() {
  ...
  try {
    ...
    console.log(`👥 Authors: ${[...new Set(commits.map(c => c.author))].length} unique contributors`);

    // Show sample of commits
    console.log("\n📋 Sample commits to be vectorized:");
    commits.slice(0, 3).forEach(commit => {
      console.log(`   ${commit.hash.substring(0, 7)} - ${commit.message.substring(0, 60)}${commit.message.length > 60 ? '...' : ''} (${commit.author})`);
    });
    if (commits.length > 3) {
    ...
    console.log(`   🏆 Success rate: ${((result.successfulVectorizations / result.totalCommits) * 100).toFixed(1)}%`);
    ...
    // Show how to search
    console.log("\n🔍 Next steps:");
    console.log("   1. Use the AOMA orchestrator to search: 'Find commits about user authentication'");
