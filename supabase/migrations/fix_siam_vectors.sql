...
-- Fix missing siam_vectors table
-- Creates the table directly with the correct schema
...
-- Create indexes
CREATE INDEX IF NOT EXISTS siam_vectors_embedding_hnsw_idx ON siam_vectors USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64);
CREATE INDEX IF NOT EXISTS siam_vectors_org_idx ON siam_vectors(organization);
CREATE INDEX IF NOT EXISTS siam_vectors_org_div_idx ON siam_vectors(organization, division);
CREATE INDEX IF NOT EXISTS siam_vectors_org_div_app_idx ON siam_vectors(organization, division, app_under_test);
CREATE INDEX IF NOT EXISTS siam_vectors_full_hierarchy_source_idx ON siam_vectors(organization, division, app_under_test, source_type);
CREATE INDEX IF NOT EXISTS siam_vectors_full_hierarchy_created_idx ON siam_vectors(organization, division, app_under_test, created_at DESC);
CREATE INDEX IF NOT EXISTS siam_vectors_metadata_idx ON siam_vectors USING gin(metadata);
