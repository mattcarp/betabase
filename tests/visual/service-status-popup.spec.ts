...
test.describe("Service Status Popup Behavior", () => {
  test("should ONLY show dropdown on hover, NEVER by default", async ({ page }) => {
    ...
    const alreadyLoggedIn = await page.locator('textarea[placeholder*="Ask"]').isVisible({ timeout: 2000 }).catch(() => false);
    ...
    expect(isVisibleOnHover,
      "Dropdown should be visible when hovering over status badge"
    ).toBe(true);
