...
test.describe('Curate Tab - Phase 1: Functional Testing', () => {
  ...
  test('1.4 - File Upload - Click to Browse', async ({ page }) => {
    ...
    const testFilePath = await createTestFile('test-document.txt', 'This is a test document for SIAM knowledge base.');
  ...
  test('1.8 - Refresh Files', async ({ page }) => {
    ...
    const refreshButton = page.locator('button[title="Refresh"]').or(page.locator('button svg[class*="RefreshCw"]').first());
  ...
  test('1.9 - Deduplicate Button', async ({ page }) => {
    ...
    const dedupeButton = page.locator('button[title="Find and remove duplicate files"]').or(
      page.locator('button').filter({ has: page.locator('svg[class*="GitMerge"]') }).first()
    );
...
test.describe('Curate Tab - Phase 2: UX & Interaction Quality', () => {
  ...
  test('2.1 - Upload Progress Feedback', async ({ page }) => {
    ...
    await expect(page.locator('text=Uploading').or(page.locator('[role="progressbar"]'))).toBeVisible({ timeout: 5000 });
  ...
  test('2.2 - Empty State Messaging', async ({ page }) => {
    ...
    await expect(page.locator('text=No files found').or(page.locator('text=No files match'))).toBeVisible();
...
test.describe('Curate Tab - Phase 5: Error Handling', () => {
  ...
  test('5.1 - Invalid File Upload', async ({ page }) => {
    ...
    await expect(page.locator('text=exceeds maximum size').or(page.locator('text=too large'))).toBeVisible({ timeout: 5000 });
  });
  ...
  test('5.2 - Network Error Simulation', async ({ page }) => {
    ...
    const refreshButton = page.locator('button').filter({ has: page.locator('svg[class*="RefreshCw"]') }).first();
    if (await refreshButton.isVisible()) {
      await refreshButton.click();
    }
