...
test.describe('Chat Progress Indicator', () => {
  ...
  test('should show context-aware progress indicator when clicking suggestion', async ({ page }) => {
  ...
  test('should position progress indicator above chat messages', async ({ page }) => {
    ...
    const progressIndicator = await page.locator('.bg-gradient-to-r.from-gray-800.to-gray-700').first();
    ...
    const progressInMessages = await messagesArea.locator('.bg-gradient-to-r.from-gray-800.to-gray-700').first();
