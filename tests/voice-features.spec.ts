...
test.describe('Voice Features Integration', () => {
  ...
  test('should display microphone button in chat input', async ({ page }) => {
    ...
    const micExists = await micButtonByTitle.isVisible() || 
  ...
  test('should display speaker button in chat input', async ({ page }) => {
    ...
    const speakerExists = await speakerButtonByTitle.isVisible() || 
  ...
  test('should inspect DOM structure for voice components', async ({ page }) => {
    ...
    const domInfo = await page.evaluate(() => {
      ...
      const voiceElements = document.querySelectorAll('[title*="record"], [title*="voice"], [title*="speak"], [title*="audio"], .lucide-mic, .lucide-volume-2');
    ...
    expect(domInfo.voiceRelated.length, 'Should find voice-related DOM elements').toBeGreaterThan(0);
  });
  ...
  test('should check for JavaScript errors preventing rendering', async ({ page }) => {
    ...
    if (errors.length > 0) {
      console.warn('Found JavaScript errors that may prevent voice buttons from rendering:', errors);
    }
  });
});