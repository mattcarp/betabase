...
test.describe('AOMA Performance Monitoring', () => {
  ...
  test('should measure AOMA query performance (cold start)', async () => {
    ...
    if (duration > PERFORMANCE_THRESHOLDS.AOMA_QUERY_COLD) {
      console.error(`❌ PERFORMANCE REGRESSION: Query took ${duration.toFixed(0)}ms (threshold: ${PERFORMANCE_THRESHOLDS.AOMA_QUERY_COLD}ms)`);
      console.error(`🔍 Expected: < ${PERFORMANCE_THRESHOLDS.AOMA_QUERY_COLD}ms`);
      console.error(`🐌 Actual: ${duration.toFixed(0)}ms`);
      console.error(`📈 Slowdown: ${((duration / PERFORMANCE_THRESHOLDS.AOMA_QUERY_COLD) * 100 - 100).toFixed(0)}% over threshold`);
    }
  ...
  test('should measure chat API performance with AOMA', async () => {
    ...
    const response = await fetch('http://localhost:3000/api/chat', {
      ...
      body: JSON.stringify({
        messages: [
          { role: 'user', content: 'Quick test query' }
        ]
      })
    });
...
test.describe('AOMA Performance Regression Detection', () => {
  ...
  test('should track performance trends', async () => {
    ...
    console.log(`Std Dev: ${Math.sqrt(results.reduce((sq, n) => sq + Math.pow(n - average, 2), 0) / results.length).toFixed(0)}ms`);
    ...
    if (average > PERFORMANCE_THRESHOLDS.AOMA_QUERY_COLD * 2) {
      ...
      console.error(`This is ${((average / PERFORMANCE_THRESHOLDS.AOMA_QUERY_COLD)).toFixed(1)}x slower than target!`);
    }
  });
});
