...
async function isApiRunning(): Promise<boolean> {
  try {
    const res = await fetch(`${KNOWLEDGE_API_URL}/health`);
    return res.ok;
  } catch {
    return false;
  }
...
describe('Knowledge API Quality', () => {
  ...
  beforeAll(async () => {
    ...
    if (!apiRunning) {
      console.warn('Knowledge API not running - skipping tests');
    }
  });

  describe('Search Quality', () => {
    it('"What is AOMA?" returns wiki documentation, not just Jira titles', async () => {
      if (!apiRunning) return;

      const response = await fetch(`${KNOWLEDGE_API_URL}/v1/knowledge/query`, {
        method: 'POST',
    ...
    it('Search returns diverse source types', async () => {
      if (!apiRunning) return;
    ...
    it('Synthesis provides useful answer, not "I dont know"', async () => {
      if (!apiRunning) return;
  ...
  describe('Performance', () => {
    it('Query with synthesis completes in <3s', async () => {
      if (!apiRunning) return;
    ...
    it('Query without synthesis completes in <1.5s', async () => {
      if (!apiRunning) return;
    ...
    it('Cached queries are fast (<100ms)', async () => {
      if (!apiRunning) return;

      // First query to warm cache
      await fetch(`${KNOWLEDGE_API_URL}/v1/knowledge/query`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  describe('Jira Detail Expand', () => {
    it('Detail endpoint returns full data for cached tickets', async () => {
      if (!apiRunning) return;

      // ITSM-48949 is in the JSON cache
      const response = await fetch(`${KNOWLEDGE_API_URL}/v1/knowledge/detail/jira/ITSM-48949`);
    ...
    it('Detail endpoint returns minimal data with Jira link for uncached tickets', async () => {
      if (!apiRunning) return;

      // ITSM-53036 is only in siam_vectors, not JSON cache
      const response = await fetch(`${KNOWLEDGE_API_URL}/v1/knowledge/detail/jira/ITSM-53036`);
    ...
    it('Jira results have expandable flag', async () => {
      if (!apiRunning) return;
  ...
  describe('Health Check', () => {
    it('Health endpoint returns service info', async () => {
      ...
      const response = await fetch(`${KNOWLEDGE_API_URL}/health`);
