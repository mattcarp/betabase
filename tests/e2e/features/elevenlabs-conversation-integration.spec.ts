...
test.describe("ElevenLabs Conversational AI Integration", () => {
  ...
  test("should establish WebSocket connection", async ({ page }) => {
    // Skip if ELEVENLABS_AGENT_ID is not configured
    const agentId = process.env.NEXT_PUBLIC_ELEVENLABS_AGENT_ID;
    test.skip(!agentId, "ELEVENLABS_AGENT_ID not configured - run with infisical run");
  ...
  test("should handle interrupt during AI speech", async ({ page }) => {
    // Skip if ELEVENLABS_AGENT_ID is not configured
    const agentId = process.env.NEXT_PUBLIC_ELEVENLABS_AGENT_ID;
    test.skip(!agentId, "ELEVENLABS_AGENT_ID not configured - run with infisical run");
