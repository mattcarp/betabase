...
test.describe("Critical Chat Paths @critical", () => {
  ...
  test("Chat interface loads correctly", async ({ authenticatedPage }) => {
    ...
    const chatElements = {
      input: authenticatedPage.locator('textarea, input[type="text"][placeholder*="Type"], [data-testid="chat-input"]').first(),
      sendButton: authenticatedPage.locator('button:has-text("Send"), button[aria-label*="Send"]').first(),
      chatContainer: authenticatedPage.locator('[class*="chat"], [class*="message"], .mac-glass').first()
    };
  ...
  test("Can send a chat message", async ({ authenticatedPage }) => {
    // Find chat input
    const chatInput = authenticatedPage.locator('textarea, input[type="text"][placeholder*="Type"], [data-testid="chat-input"]').first();
    ...
    // Send message
    const sendButton = authenticatedPage.locator('button:has-text("Send"), button[aria-label*="Send"], button[type="submit"]').first();
  ...
  test("New chat button creates fresh session", async ({ authenticatedPage }) => {
    ...
    const newChatButton = authenticatedPage.locator('button:has-text("New Chat"), button:has-text("New Conversation"), button[aria-label*="New"]').first();
    
    if (await newChatButton.isVisible()) {
      await newChatButton.click();
      
      // First message should no longer be visible
      await expect(authenticatedPage.locator(`text="${firstMessage}"`)).not.toBeVisible({
        timeout: 5000
      }).catch(() => {
  ...
  test("Voice input button is accessible", async ({ authenticatedPage }) => {
    ...
    const voiceButton = authenticatedPage.locator('button[aria-label*="Voice"], button[aria-label*="Microphone"], button:has([class*="microphone"]), button:has([class*="mic"])').first();
    ...
    if (await voiceButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      for (const selector of activeIndicators) {
        if (await authenticatedPage.locator(selector).isVisible({ timeout: 3000 }).catch(() => false)) {
          foundActive = true;
          break;
        }
      }
