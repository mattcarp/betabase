...
test.describe("Critical File Upload Paths @critical", () => {
  ...
  test.beforeEach(async ({ authenticatedPage }) => {
    ...
    await authenticatedPage
      .goto("/curate")
      .catch(() => authenticatedPage.goto("/files").catch(() => authenticatedPage.goto("/")));
  });
  ...
  test("Drag and drop upload works", async ({ authenticatedPage }) => {
    ...
    const dropzone = authenticatedPage
      .locator('[class*="dropzone"], [class*="upload"], [data-testid="dropzone"]')
      .first();
    ...
    await authenticatedPage.evaluateHandle(
      async ({ dropzoneSelector, fileName, fileContent }) => {
    ...
    await expect(authenticatedPage.locator("text=/drag-test.txt|File.*added/i"))
      .toBeVisible({
        timeout: 10000,
      })
      .catch(() => {
  ...
  test("File size validation works", async ({ authenticatedPage }) => {
    ...
    try {
      ...
      await Promise.race([
        ...
        authenticatedPage
          .locator(`text="${path.basename(largeFilePath)}"`)
