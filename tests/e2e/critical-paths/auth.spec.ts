...
test.describe("Critical Auth Paths @critical", () => {
  ...
  test("Multiple valid email domains accepted", async ({ page }) => {
    ...
    for (const email of validEmails) {
      ...
      const response = await page.waitForSelector(
        "text=/Magic link sent|Check your email|Rate limit|Try again/i",
        {
          timeout: 30000,
        }
      );
  ...
  test("Logout flow works correctly", async ({ authenticatedPage }) => {
    const authHelper = new (await import("../../../fixtures/auth.fixture")).AuthHelper(
      authenticatedPage
    );
  ...
  test("Session persistence across page reloads", async ({ authenticatedPage }) => {
    ...
    const authenticatedElement = authenticatedPage
      .locator('.mac-professional, [data-testid="user-menu"]')
      .first();
