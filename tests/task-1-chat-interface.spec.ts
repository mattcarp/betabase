...
test.describe("Task 1: SIAM Chat Interface", () => {
  ...
  test("3-panel layout is implemented", async ({ page }) => {
    ...
    const panels = await page
      .locator('[class*="panel"], [class*="sidebar"], [class*="main"], [class*="conversation"]')
      .count();
    ...
    const hasLayout = await page.evaluate(() => {
      const elements = document.querySelectorAll(
        '[class*="layout"], [class*="grid"], [class*="flex"]'
      );
  ...
  test("JARVIS theme integration", async ({ page }) => {
    ...
    const hasJarvisTheme = await page.evaluate(() => {
      const elements = document.querySelectorAll(
        '[class*="glass"], [class*="jarvis"], [class*="mac-"], [class*="blur"]'
      );
      return elements.length > 0;
    });
  ...
  test("Shadcn components are used", async ({ page }) => {
    ...
    const hasShadcnComponents = await page.evaluate(() => {
      const elements = document.querySelectorAll(
        '[class*="ui-"], [class*="button"], [class*="card"], [class*="dialog"]'
      );
      return elements.length > 0;
    });
  ...
  test("Connection status indicators", async ({ page }) => {
    ...
    const statusIndicators = await page
      .locator(
        '[class*="status"], [class*="connection"], [class*="indicator"], [data-testid*="status"]'
      )
      .count();
  ...
  test("Responsive design", async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      console.log(
        `${viewport.name} (${viewport.width}px): ${isResponsive ? "✓ Responsive" : "✗ Overflow"}`
      );
    }
  });
