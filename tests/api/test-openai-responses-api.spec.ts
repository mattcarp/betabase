...
test.describe('OpenAI Responses API Migration', () => {
  test('chat API should use native OpenAI streaming', async ({ request }) => {
    ...
    const response = await request.post('http://localhost:3000/api/chat', {
      data: {
        messages: [
          { role: 'user', content: 'Hello, test message for OpenAI Responses API' }
        ],
        model: 'gpt-4o-mini',
        temperature: 0.7
      }
  ...
  test('chat interface should work with new API', async ({ page }) => {
    ...
    const chatInput = page.locator('textarea[placeholder*="Message"], input[placeholder*="Message"], textarea[placeholder*="Type"], input[placeholder*="Type"]').first();
    ...
    if (await chatInput.isVisible()) {
      ...
      const messageContent = page.locator('[class*="message"], [class*="response"], [class*="assistant"]');
  ...
  test('verify knowledge elements are included', async ({ request }) => {
    ...
    const response = await request.post('http://localhost:3000/api/chat', {
      data: {
        messages: [
          { role: 'user', content: 'Tell me about AOMA' }
        ],
        model: 'gpt-4o-mini',
        temperature: 0.7
      },
