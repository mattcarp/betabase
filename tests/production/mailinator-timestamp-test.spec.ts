...
test.describe('Production Mailinator Test with Timestamp Check', () => {
  ...
  test('Check build timestamp and test Mailinator auth', async ({ page, context }) => {
    ...
    const timestampElement = await page.locator('text=/Built.*GMT/i, text=/v[0-9]+\.[0-9]+\.[0-9]+.*Built/i').first();
    ...
    const successMessage = page.locator('text=/check your email/i, text=/magic link sent/i, text=/email sent/i');
    ...
    if (await successMessage.isVisible()) {
      ...
      if (emailRows > 0) {
        ...
        const links = await magicLinkFrame.locator('a[href*="magic"], a[href*="verify"], a[href*="confirm"]').count();
    ...
    console.log('- Mailinator Inbox: https://www.mailinator.com/v4/public/inboxes.jsp?to=siam-test-x7j9k2p4');
  });
});