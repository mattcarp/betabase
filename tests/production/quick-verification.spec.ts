...
test.describe('Quick Production Verification', () => {
  ...
  test('File upload interface is accessible', async ({ page }) => {
    ...
    const verificationVisible = await page.locator('text=/check your email/i').isVisible().catch(() => false);
    ...
    if (verificationVisible) {
      ...
      const chatInterface = await page.locator('[data-testid="chat-interface"], .chat-panel, .mac-professional').isVisible().catch(() => false);
      
      if (chatInterface) {
        console.log('✅ Chat interface accessible');
        
        // Look for file upload button
        const uploadButton = await page.locator('button[aria-label*="upload"], input[type="file"], [data-testid*="upload"]').isVisible().catch(() => false);
