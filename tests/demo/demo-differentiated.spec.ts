...
test.describe('SIAM Demo - Points of Differentiation', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const isLoginPage = await page.getByText(/sign in|log in|email/i).isVisible().catch(() => false);
    if (isLoginPage) {
  ...
  test('Full Differentiated Demo', async ({ page }) => {
    ...
    console.log('💬 OPENING: But testing is a different video. Today: unique chatbot capabilities...\n');
    
    await pause(page, DEMO_CONFIG.mediumPause);
    
    const chatInput = page.getByPlaceholder(/ask|type|message|chat/i)
    ...
    await typeSlowly(chatInput, 'Generate a system architecture diagram for AOMA showing all integration points');
    ...
    await typeSlowly(chatInput, 'Analyze the dependencies and integration health between AOMA and downstream systems');
