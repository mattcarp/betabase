...
test.describe("AI Insights UI (Task 40.5)", () => {
  test("Chat interface loads successfully", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="chat-interface"], .mac-chat, .ai-chat, textarea[placeholder*="message"], textarea[placeholder*="Message"], textarea[placeholder*="Type"], input[placeholder*="message"]')).toBeVisible({ timeout: 10000 });
  });
  ...
  test("AOMA Response component renders", async ({ page }) => {
    ...
    const hasAOMAElements = await page.evaluate(() => {
      ...
      const elements = document.querySelectorAll('.aoma-response, [class*="aoma"], [class*="AOMA"]');
  ...
  test("Connection status indicator shows MCP status", async ({ page }) => {
    ...
    const connectionIndicator = page.locator('[data-testid="connection-status"], [class*="connection"], [class*="status"], [class*="indicator"]').first();
  ...
  test("Chat input accepts and sends messages", async ({ page }) => {
    ...
    // Find the chat input
    const chatInput = page.locator('textarea[placeholder*="message"], textarea[placeholder*="Message"], textarea[placeholder*="Type"], input[placeholder*="message"]').first();
    ...
    const sendButton = page.locator('button[type="submit"], button:has-text("Send"), button[aria-label*="send"], button[aria-label*="Send"]').first();
...
test.describe("Task Verification", () => {
  ...
  test("Task 40.5: AI Insights UI is implemented", async ({ page }) => {
    ...
    const hasChat = await page.locator('textarea[placeholder*="message"], textarea[placeholder*="Message"], input[placeholder*="message"]').count() > 0;
    const hasUI = await page.locator('[class*="mac-"], [class*="ui-"], [class*="chat"]').count() > 0;
