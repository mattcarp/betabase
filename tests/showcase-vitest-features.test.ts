...
describe("🔐 Authentication System", () => {
  describe("Magic Link Authentication", () => {
    ...
    it("should validate authorized emails", () => {
      const authorizedEmails = ["matt@mattcarpenter.com", "fiona@fionaburgess.com"];

      const isAuthorized = (email: string) => {
        return authorizedEmails.includes(email) || email.endsWith("@sonymusic.com");
      };
  ...
  describe("Real AOMA Responses (No Hallucination)", () => {
        ...
        results: [
          {
            id: "aoma-doc-1",
            content:
              "AOMA (Asset Online Management Application) is Sony Music's digital asset management system",
            metadata: {
    ...
    it("should validate answer against knowledge base", async () => {
      ...
      const validateAnswer = (answer: string, topic: string) => {
        ...
        return facts.some((fact) => answer.toLowerCase().includes(fact.toLowerCase()));
      };
...
describe("🎨 Vitest Advanced Features Showcase", () => {
  ...
  describe("Snapshot Testing", () => {
    it("should match AOMA response structure", () => {
      const aomaResponse = {
        ...
        sources: [{ id: "doc-1", title: "AOMA User Guide", relevance: 0.95 }],
        metadata: {
          processingTime: 145,
          model: "gpt-4",
  ...
  describe("Conditional Tests", () => {
    ...
    it.runIf(process.env.BACKEND_AVAILABLE === "true")(
      "should test against live backend",
      async () => {
