...
export async function navigateTo(page: Page, url: string, waitFor: 'load' | 'domcontentloaded' | 'networkidle' = 'networkidle') {
...
export async function waitForText(page: Page, selector: string, expected: string, timeoutMs = 10_000) {
...
export async function bypassDevAuth(page: Page) {
  ...
  await page.addInitScript(() => {
    try { localStorage.setItem('siam_dev_auth', 'true'); } catch {}
  });
}
