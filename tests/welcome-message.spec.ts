...
test.describe("Welcome Message", () => {
  test("should display 'Welcome to The Betabase' not 'Welcome to the New Conversation'", async ({ page }) => {
    ...
    await expect(page.locator("h2:has-text('Welcome to The Betabase')")).toBeVisible({ timeout: 5000 });
    ...
    const welcomeHeading = await page.locator("h2").filter({ hasText: /Welcome to/ }).textContent();
    expect(welcomeHeading).toBe("Welcome to The Betabase");
    
    // Verify the description text is also correct
    await expect(page.locator("text=Ready to assist you with advanced AI capabilities")).toBeVisible();
    await expect(page.locator("text=Again, don't be a dick")).toBeVisible();
  });

  test("should maintain correct welcome message after conversation interactions", async ({ page }) => {
    await page.goto("http://localhost:3000");
    await page.waitForSelector("h2", { timeout: 10000 });
    ...
    const suggestion = page.locator("button").filter({ hasText: "How does the new automated QC" }).first();
    if (await suggestion.isVisible()) {
      ...
      const newConvoButton = page.locator("button").filter({ hasText: /New Conversation|New Thread|New Chat/ }).first();
      if (await newConvoButton.isVisible()) {
        await newConvoButton.click();
        await page.waitForTimeout(500);
        
        // Verify the welcome message is still correct
        const welcomeVisible = await page.locator("h2:has-text('Welcome to The Betabase')").isVisible();
        if (welcomeVisible) {
  ...
  test("should never show 'Welcome to the New Conversation' text", async ({ page }) => {
    ...
    const welcomeArea = page.locator("div").filter({ has: page.locator("h2").filter({ hasText: /Welcome to/ }) }).first();
