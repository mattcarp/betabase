...
const results = await fetch(`https://api.openai.com/v1/vector_stores/${VECTOR_STORE_ID}/search`, {
  method: "POST",
  headers: {
...
const context = highQualityResults
  .slice(0, 3)
...
const thresholds = {
  rapid: 0.8, // Only highest confidence
  focused: 0.7, // Good confidence
  comprehensive: 0.6, // Include background context
};

// Filter results by strategy
const filtered = results.data.filter((r) => r.score >= thresholds[strategy]);
```
