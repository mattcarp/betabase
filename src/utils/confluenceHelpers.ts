...
export function storageToMarkdown(storageHtml: string): string {
  ...
  let html = storageHtml.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, '');
...
export function buildSourceId(pageId: string | number, version?: { number?: number } | number): string {
