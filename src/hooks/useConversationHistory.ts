...
export const useConversationHistory = () => {
  ...
  const updateConversation = useCallback((conversationId: string, messages: any[]) => {
      ...
      if (conversationIndex >= 0) {
        ...
        const newConversation: ConversationData = {
          ...
          title: messages.length > 0 && messages[0].role === 'user' 
            ? messages[0].content.substring(0, 50).trim() + (messages[0].content.length > 50 ? '...' : '')
            : 'New Conversation',
        ...
        const newItem: ConversationItem = {
          id: conversationId,
          title: newConversation.title,
          lastMessage: newConversation.messages.length > 0 
            ? newConversation.messages[newConversation.messages.length - 1].content.substring(0, 100) + '...'
            : 'New conversation',
        ...
        setConversations(prev => 
          prev.map(item => 
            item.id === conversationId 
              ? { ...item, isPinned: conversation.isPinned }
              : item
          )
        );
      }
