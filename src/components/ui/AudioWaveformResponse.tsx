...
const WaveformVisualization: React.FC<{
  ...
  return (
    <canvas
      ...
      width={800}
      height={120}
      className="w-full h-20 bg-card/50 rounded border border-border/50"
    />
  );
};
    ...
    <div className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 rounded-xl p-6 border border-border/50 backdrop-blur-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-muted/50 rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-2">RMS Energy</div>
        ...
        <div className="bg-muted/50 rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-2">SNR</div>
        ...
        <div className="bg-muted/50 rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-2">Voice Activity</div>
          <div
      ...
      {data.transcription && (
        <div className="bg-card/50 rounded-lg p-4 mb-4">
          ...
          <p className="mac-body text-foreground leading-relaxed">{data.transcription.text}</p>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {Object.entries(data.analysis.emotionScore).map(([emotion, score]) => (
              <div key={emotion} className="text-center">
                ...
                <div className="text-xs text-foreground">{(score * 100).toFixed(0)}%</div>
              </div>
            ))}
          </div>
        </div>
