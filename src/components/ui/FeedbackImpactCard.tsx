    ...
    try {
      ...
      const corrections = feedback.filter(
        (f: { suggested_correction?: string; thumbs_up?: boolean; feedback_type?: string }) =>
          f.suggested_correction || f.thumbs_up === false || f.feedback_type === "correction"
      );
    ...
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <RefreshCw
                className={cn(
                  "h-4 w-4 text-[var(--mac-text-secondary)]",
            ...
            >
              <TrendingUp className="h-3 w-3 mr-1" />+{impactData.accuracyImprovement}% this month
            </Badge>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Progress value={impactData.currentAccuracy} className="h-2" />
          <p className="text-xs text-[var(--mac-text-tertiary)]">
            Improved from 83% to {impactData.currentAccuracy}% based on your feedback
          </p>
        </div>
          ...
          {loading ? (
            <div className="space-y-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-16 rounded-lg bg-[var(--mac-surface-elevated)] animate-pulse"
                />
              ))}
            </div>
            ...
            impactData.recentCorrections.map((correction) => (
              ...
              >
                <div className="space-y-1 flex-1">
                  ...
                  <p className="text-xs text-[var(--mac-text-tertiary)]">{correction.timestamp}</p>
                </div>
                <Badge
                  variant="secondary"
  ...
  return (
    <div className="text-center p-4 rounded-lg bg-[var(--mac-surface-elevated)] border border-[var(--mac-utility-border)]">
      ...
      {loading ? (
        ...
        <div className="text-2xl font-light text-[var(--mac-text-primary)]">{value}</div>
      )}
...
function CycleStep({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
