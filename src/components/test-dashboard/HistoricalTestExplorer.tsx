  ...
  return (
    <Card className="h-full flex flex-col mac-card" data-test-id="historical-test-explorer">
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-[var(--mac-text-secondary)] font-light">
      ...
      <CardContent className="flex-1 flex flex-col space-y-4 min-h-0">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--mac-text-secondary)]" />
            <Input
              placeholder="Search tests..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-9 bg-[var(--mac-surface-elevated)] border-[var(--mac-border)] text-white placeholder:text-[var(--mac-text-muted)] focus:border-[var(--mac-primary-blue-400)]"
          ...
          <select
            ...
            className="px-3 py-2 bg-[var(--mac-surface-elevated)] border border-[var(--mac-border)] rounded-md text-white text-sm font-light"
          ...
          <Button 
            ...
            variant="outline" 
            size="sm"
            className="border-[var(--mac-border)] hover:bg-[var(--mac-surface-elevated)] hover:border-[var(--mac-primary-blue-400)] text-white font-light"
          ...
          <Button 
            ...
            className="text-[var(--mac-text-secondary)] hover:text-white font-light"
          >
        ...
        {/* Quick Stats */}
        <div className="flex gap-4 text-sm text-[var(--mac-text-secondary)] font-light">
        ...
        <ScrollArea className="flex-1 min-h-[300px] -mx-4 px-4" data-test-id="test-list">
          {loading ? (
            <div className="space-y-3">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="h-24 bg-[var(--mac-surface-elevated)] rounded-lg animate-pulse" />
              ))}
            </div>
          ) : tests.length === 0 ? (
            <div className="text-center py-12 text-[var(--mac-text-muted)]">
                ...
                <div
                  ...
                  className={cn(
                    ...
                    selectedTest?.id === test.id
                      ? "bg-[var(--mac-primary-blue-400)]/10 border-[var(--mac-primary-blue-400)]/50"
                      : "bg-[var(--mac-surface-elevated)] border-[var(--mac-border)] hover:bg-[var(--mac-state-hover)] hover:border-[var(--mac-utility-border-elevated)]"
                  )}
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-[var(--mac-text-secondary)] font-light line-clamp-2">
                        {test.description || "No description available"}
                      </p>
                      <div className="flex items-center gap-3 mt-2 text-xs text-[var(--mac-text-muted)] font-light">
                        <Badge variant="outline" className="text-xs border-[var(--mac-utility-border-elevated)] text-[var(--mac-text-secondary)]">
                          {test.app_under_test}
                        </Badge>
                        <Badge variant="outline" className="text-xs border-[var(--mac-utility-border-elevated)] text-[var(--mac-text-secondary)]">
                          {test.category}
                        ...
                        {test.execution_count === 0 && (
                          <span className="flex items-center gap-1 text-[var(--mac-text-muted)]">
                    ...
                    <div className="text-right text-xs text-[var(--mac-text-muted)] shrink-0 font-light">
                      <div>#{test.id}</div>
        ...
        {/* Pagination */}
        <div className="flex items-center justify-between pt-2 border-t border-[var(--mac-border)]">
          <Button
            variant="ghost"
            size="sm"
            disabled={pagination.page <= 1}
            onClick={() => goToPage(pagination.page - 1)}
            className="text-[var(--mac-text-secondary)] font-light hover:text-white"
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    pageNum === pagination.page 
                      ...
                      : "text-[var(--mac-text-secondary)] hover:text-white"
                  )}
                >
                  {pageNum}
                </Button>
        ...
        {selectedTest && (
          <div className="p-4 bg-[var(--mac-surface-elevated)] rounded-lg border border-[var(--mac-border)] mt-2" data-test-id="test-detail">
            <div className="flex items-start justify-between mb-3">
              ...
              <Button
                ...
                className="text-[var(--mac-text-muted)] hover:text-white -mr-2 -mt-2"
              >
                ×
              </Button>
            </div>
            <p className="text-sm text-[var(--mac-text-secondary)] font-light mb-3">{selectedTest.description}</p>
            
            <div className="grid grid-cols-2 gap-4 text-sm font-light">
              <div>
                <span className="text-[var(--mac-text-muted)]">App:</span>
              ...
              <div>
                <span className="text-[var(--mac-text-muted)]">Executions:</span>
                <span className="ml-2 text-white">{selectedTest.execution_count}</span>
              </div>
              <div>
                <span className="text-[var(--mac-text-muted)]">Pass/Fail:</span>
                <span className="ml-2">
                  <span className="text-emerald-400">{selectedTest.pass_count}</span>
                  <span className="text-[var(--mac-text-muted)] opacity-50"> / </span>
                  <span className="text-red-400">{selectedTest.fail_count}</span>
                </span>
              </div>
              <div>
                <span className="text-[var(--mac-text-muted)]">Confidence:</span>
                <span className="ml-2">{getConfidenceBadge(selectedTest.base_confidence)}</span>
              </div>
              <div>
                <span className="text-[var(--mac-text-muted)]">Jira Links:</span>
                <span className="ml-2 text-white">{selectedTest.jira_ticket_count}</span>
              </div>
            </div>

            {selectedTest.preconditions && (
              <div className="mt-3 pt-3 border-t border-[var(--mac-border)]">
                <span className="text-[var(--mac-text-muted)] text-sm font-light">Preconditions:</span>
                <p className="text-sm text-[var(--mac-text-secondary)] font-light mt-1">{selectedTest.preconditions}</p>
              </div>
            )}
            ...
            <div className="flex gap-2 mt-4">
              ...
              <Button 
                size="sm" 
                variant="outline" 
                className="border-[var(--mac-utility-border-elevated)] text-white hover:bg-[var(--mac-surface-elevated)] hover:border-[var(--mac-primary-blue-400)] font-light"
                onClick={async () => {
            ...
            {aiConfidence && (
              <div className="mt-3 p-3 bg-[var(--mac-surface-background)] rounded border border-[var(--mac-border)]">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[var(--mac-text-muted)] text-sm font-light">AI Analysis:</span>
            ...
            {generatedCode && (
              <div className="mt-3 p-3 bg-[var(--mac-surface-background)] rounded border border-[var(--mac-border)]">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[var(--mac-text-muted)] text-sm font-light">Generated Playwright Test:</span>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => {
                ...
                <pre className="text-xs text-[var(--mac-text-secondary)] overflow-x-auto max-h-64 p-2 bg-[var(--mac-surface-bg)] rounded font-mono">
