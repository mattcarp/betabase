...
export function HistoricalTestExplorer() {
  ...
  const getConfidenceBadge = (confidence: number) => {
    if (confidence >= 0.8) {
      return <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30">High ({Math.round(confidence * 100)}%)</Badge>;
    } else if (confidence >= 0.5) {
      return <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30">Medium ({Math.round(confidence * 100)}%)</Badge>;
    } else {
      return <Badge className="bg-zinc-500/20 text-zinc-400 border-zinc-500/30">Low ({Math.round(confidence * 100)}%)</Badge>;
    }
  };
  ...
  return (
    <Card className="h-full flex flex-col bg-zinc-900/50 border-zinc-800" data-test-id="historical-test-explorer">
      ...
      <CardContent className="flex-1 flex flex-col space-y-4 min-h-0">
        ...
        <div className="flex gap-4 text-sm text-zinc-400">
          ...
          <span className="flex items-center gap-1">
            <TrendingUp className="h-4 w-4 text-emerald-400" /> Page {pagination.page} of {pagination.totalPages}
          </span>
        </div>

        {/* Test List */}
        <ScrollArea className="flex-1 -mx-4 px-4" data-test-id="test-list">
          {loading ? (
        ...
        {selectedTest && (
          <div className="p-4 bg-zinc-800/80 rounded-lg border border-zinc-700 mt-2" data-test-id="test-detail">
            <div className="flex items-start justify-between mb-3">
              <h3 className="font-semibold text-zinc-100">{selectedTest.test_name}</h3>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setSelectedTest(null)}
                className="text-zinc-400 -mr-2 -mt-2"
              >
                ×
              </Button>
            </div>
            <p className="text-sm text-zinc-300 mb-3">{selectedTest.description}</p>
            
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-zinc-500">App:</span>
                <span className="ml-2 text-zinc-200">{selectedTest.app_under_test}</span>
              </div>
              <div>
                <span className="text-zinc-500">Category:</span>
                <span className="ml-2 text-zinc-200">{selectedTest.category}</span>
              </div>
              <div>
                <span className="text-zinc-500">Executions:</span>
                <span className="ml-2 text-zinc-200">{selectedTest.execution_count}</span>
              </div>
              <div>
              ...
              <div>
                <span className="text-zinc-500">Jira Links:</span>
                <span className="ml-2 text-zinc-200">{selectedTest.jira_ticket_count}</span>
              </div>
            </div>

            {selectedTest.preconditions && (
              <div className="mt-3 pt-3 border-t border-zinc-700">
                <span className="text-zinc-500 text-sm">Preconditions:</span>
                <p className="text-sm text-zinc-300 mt-1">{selectedTest.preconditions}</p>
              </div>
            )}
