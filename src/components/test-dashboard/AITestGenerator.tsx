...
describe('API Integration Tests', () => {
  ...
  const handleGenerate = async () => {
    if (recordScreencast) {
      ...
      // AI test generation mode - call the real API
      setIsGenerating(true);
      ...
      try {
        const response = await fetch("/api/test/generate-ai", {
          ...
          body: JSON.stringify({
            ...
            targetUrl: targetUrl.startsWith("http") ? targetUrl : undefined,
          }),
        });
        ...
        if (!response.ok || !result.success) {
          throw new Error(result.error || result.details || "Failed to generate test");
        }

        const newTest: GeneratedTest = {
          id: Date.now().toString(),
          name: result.testName || "Generated Test Suite",
          type: testType,
          language: language,
          description: `AI-generated ${testType} test based on: ${prompt}`,
          coverage: extractCoverage(result.testCode),
          timestamp: new Date(),
        ...
        toast({
          ...
          description: `Generated ${testType} test using AI`,
        });
...
/**
 * Extract coverage areas from generated test code by analyzing the code content
 */
function extractCoverage(code: string): string[] {
  ...
  if (code.includes("page.click") || code.includes("getByRole") || code.includes("getByText")) {
    coverage.push("User Interactions");
  }
  if (code.includes("expect(") || code.includes("toHave") || code.includes("toBe")) {
    coverage.push("Assertions");
  }
  if (code.includes("fill") || code.includes("type") || code.includes("input")) {
    coverage.push("Form Input");
  }
  if (code.includes("waitFor") || code.includes("waitUntil")) {
    coverage.push("Async Handling");
  }
  if (code.includes("screenshot") || code.includes("visual")) {
    coverage.push("Visual Verification");
  }
  if (code.includes("API") || code.includes("fetch") || code.includes("request")) {
    coverage.push("API Testing");
  }
  if (code.includes("error") || code.includes("Error") || code.includes("catch")) {
    coverage.push("Error Handling");
  }

  // Default coverage if nothing detected
  if (coverage.length === 0) {
    coverage.push("Basic Test Coverage");
  }
