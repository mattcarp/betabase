...
export class ExplicitContentDetector {
  ...
  private readonly explicitCategories = {
    ...
    strongLanguage: [
      ...
      'damn', 'goddamn', 'hell', 'bitch', 'bastard', 'piss',
      'ass', 'asshole', 'crap', 'cock', 'dick', 'pussy'
    ],
    
    // RIAA Category 2: Sexual Content
    sexualContent: [
      'sex', 'sexual', 'porn', 'pornography', 'orgasm', 'masturbate',
      'erotic', 'horny', 'kinky', 'fetish', 'bondage', 'oral',
      'anal', 'vagina', 'penis', 'breast', 'nipple', 'nude', 'naked'
    ],
    
    // RIAA Category 3: Violence
    violence: [
      'kill', 'murder', 'death', 'die', 'blood', 'gun', 'shoot',
      'stab', 'knife', 'weapon', 'violence', 'violent', 'fight',
      'beat', 'punch', 'attack', 'assault', 'rape', 'abuse'
    ],
    
    // RIAA Category 4: Substance Abuse
    substanceAbuse: [
      'drugs', 'drug', 'cocaine', 'heroin', 'marijuana', 'weed',
      'meth', 'crack', 'pill', 'pills', 'addiction', 'high',
      'stoned', 'drunk', 'alcohol', 'beer', 'vodka', 'whiskey'
    ],
    
    // Music-specific explicit indicators
    musicExplicit: [
      'explicit', 'parental advisory', 'uncensored', 'uncut',
      'clean version', 'radio edit', 'explicit version'
    ]
  };
   ...
   */
  async detectExplicitContent(text: string, audioMetadata?: {
    artist?: string;
    title?: string;
    album?: string;
  ...
  private analyzeContext(text: string): { explicit: boolean; reasoning: string[] } {
    ...
    for (const [theme, pattern] of Object.entries(themes)) {
      const matches = text.match(pattern);
      if (matches && matches.length > 2) { // Multiple references
        themeCount++;
        explicitScore += 0.15;
   ...
   */
  private categorizeContent(confidence: number, explicitWords: string[]): ExplicitDetectionResult['category'] {
