...
export class MultiRepoIndexer {
  ...
  async indexRepository(repoPath: string): Promise<IndexSummary> {
    ...
    const batchSize = Math.max(1, parseInt(process.env.VECTOR_FILE_BATCH_SIZE || process.env.VECTOR_BATCH_SIZE || '100', 10));
    ...
    for (let i = 0; i < changedFiles.length; i += batchSize) {
      ...
      const analyses = await Promise.all(batchFiles.map(file => limit(() => this.analyzer.analyzeFile(repoPath, file.absolutePath, file.relativePath, tag))));
      const vectors = analyses
        .filter(Boolean)
        .flatMap(a => {
          totalChunks += (a as any).chunks.length;
          return (a as any).chunks as Array<{ content: string; sourceType: 'git'; sourceId: string; metadata: Record<string, any> }>;
        });
  ...
  async indexAll({ includeCommits = true, includeFiles = true } = {}) {
    ...
    if (repos.length === 0) {
      throw new Error('No repositories configured. Set GIT_FRONTEND_REPO_PATH/GIT_BACKEND_REPO_PATH or GIT_ADDITIONAL_REPOS.');
    }

    const summaries: IndexSummary[] = [];
    const errors: Array<{ repo: string; error: string }> = [];

    const globalLimit = pLimit(Math.max(1, parseInt(process.env.GIT_GLOBAL_CONCURRENCY || '2', 10)));
    await Promise.all(repos.map(repo => globalLimit(async () => {
      try {
        if (includeFiles) {
