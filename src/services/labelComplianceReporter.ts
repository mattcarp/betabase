...
export class LabelComplianceReporter {
  ...
  private checkRetailerCompliance(tracks: TrackAnalysisReport[]) {
    ...
    return {
      ...
      riaaCompliant: tracks.every(
        (t) => !t.requiresParentalAdvisory || t.explicitAnalysis.riaaCompliant
      ),

      // Spotify: Explicit flag accuracy
      spotifyCompliant: tracks.every(
        (t) => t.retailerClassification !== "explicit" || t.explicitAnalysis.confidence > 0.5
      ),

      // Apple Music: Conservative explicit flagging
      appleMusicCompliant: tracks.every(
        (t) => t.explicitAnalysis.confidence < 0.3 || t.retailerClassification === "explicit"
      ),

      // Amazon Music: Similar to Spotify
      amazonMusicCompliant: tracks.every(
        (t) => t.retailerClassification !== "explicit" || t.explicitAnalysis.confidence > 0.4
      ),

      // Physical media: RIAA compliance
      physicalMediaCompliant:
        explicitTracks.length === 0 || explicitTracks.every((t) => t.requiresParentalAdvisory),
    };
  }
...
<labelComplianceReport>
  ...
  private printExecutiveSummary(report: BatchAnalysisReport): void {
    ...
    console.log(
      `   Clean Tracks: ${report.summary.cleanTracks} (${((report.summary.cleanTracks / report.totalTracks) * 100).toFixed(1)}%)`
    );
    console.log(
      `   Explicit Tracks: ${report.summary.explicitTracks} (${((report.summary.explicitTracks / report.totalTracks) * 100).toFixed(1)}%)`
    );
    ...
    console.log(
      `   Apple Music Ready: ${report.compliance.appleMusicCompliant ? "✅ YES" : "❌ NO"}`
    );
    console.log(
      `   Amazon Music Ready: ${report.compliance.amazonMusicCompliant ? "✅ YES" : "❌ NO"}`
    );
    console.log(
      `   Physical Media Ready: ${report.compliance.physicalMediaCompliant ? "✅ YES" : "❌ NO"}`
    );
    console.log("");
    ...
    if (report.summary.explicitTracks > 0) {
      ...
      report.tracks
        ...
        .forEach((track) => {
          ...
          console.log(
            `     Classification: EXPLICIT (${(track.explicitAnalysis.confidence * 100).toFixed(1)}% confidence)`
          );
          console.log(
            `     RIAA Label: ${track.requiresParentalAdvisory ? "REQUIRED" : "Not Required"}`
          );
          console.log(
            `     Edited Version: ${track.recommendations.editedVersionNeeded ? "RECOMMENDED" : "Optional"}`
          );
          if (track.recommendations.regionalRestrictions.length > 0) {
            console.log(
              `     Regional Notes: ${track.recommendations.regionalRestrictions.join(", ")}`
            );
          }
          console.log("");
        });
