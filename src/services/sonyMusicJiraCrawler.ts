...
async function scrapeIssue(page: Page, url: string) {
  ...
  const title =
    (await page
      ...
      .catch(() => null)) ||
    (await page
      .locator('[data-test-id="issue.views.issue-base.foundation.summary.heading"]')
      .textContent()
      .catch(() => null)) ||
    "";
  ...
  const description =
    (await page
      .locator("#description-val")
      .textContent()
      .catch(() => null)) ||
    (await page
      .locator('[data-test-id="issue.views.field.rich-text.editor.ui.content"] div')
      .first()
      .textContent()
      .catch(() => null)) ||
    "";
  ...
  const status =
    (await page
      ...
      .catch(() => null)) ||
    (await page
      .locator('[data-test-id="issue.views.issue-base.foundation.status.status-field"]')
      .textContent()
      .catch(() => null)) ||
    "";
  ...
  const priority =
    (await page
      .locator("#priority-val")
      .textContent()
      .catch(() => null)) ||
    (await page
      .locator('[data-test-id="issue.views.issue-base.foundation.priority.priority-field"]')
      .textContent()
      .catch(() => null)) ||
    "";
  ...
  const commentNodes = await page.$$(
    '[data-test-id="comment"] , #issue_actions_container .actionContainer'
  );
...
export async function crawlProjects(options: CrawlOptions = {}) {
  const projects =
    options.projects && options.projects.length ? options.projects : DEFAULT_PROJECTS;
  ...
  try {
    ...
    await page.goto(`${JIRA_BASE}/issues/?jql=${encodeURIComponent(jql)}`, {
    ...
    for (const link of links) {
      ...
      await upsertJiraTicket(issue.issueKey, issue.title, issue.description, embedding, {
        url: link,
      ...
      await upsertJiraTicketEmbedding(issue.issueKey, issue.title, embedding, {
        project: issue.projectKey,
        status: issue.status,
        sony_music: true,
      });
