...
export class UnifiedTestIntelligence {
  ...
  constructor() {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      throw new Error(
        "[UnifiedTestIntelligence] Supabase credentials missing. Set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY."
      );
    }
  ...
  async gatherAUTIntelligence(appUrl: string = this.AOMA_STAGING_URL) {
    try {
      ...
      await this.supabaseIntegration.storeFirecrawlAnalysis({
        ...
        // analyzed_at: new Date().toISOString(), // Auto-generated by database
      } as any);
  ...
  async processTestFailure(testResult: any) {
    try {
      ...
      if (!aomaResponse) {
        const orchestratorResponse = await (aomaOrchestrator as any).orchestrateQuery(
  ...
  async searchTestKnowledge(
    ...
    try {
      ...
      const results = await (this.supabaseIntegration as any).searchTestKnowledge(
        query,
        options?.sources,
  ...
  private async getCachedAnalysis(url: string) {
    try {
      const result = await (this.supabaseIntegration as any).getFirecrawlAnalysis(url);
  ...
  private async storeTestKnowledge(knowledge: KnowledgeEntry[]) {
    try {
      for (const entry of knowledge) {
        await (this.supabaseIntegration as any).storeTestKnowledge(entry);
      }
    } catch (error) {
