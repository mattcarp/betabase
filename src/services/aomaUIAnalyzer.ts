...
export class AomaUIAnalyzer {
  ...
  private analyzeFromHtml(url: string, html: string): UIAnalysisOutput {
    ...
    const formMatches = htmlStr.match(/<form[^>]*>.*?<\/form>/gsi) || []
    ...
    return { title, elements, selectors, navigationPaths: [...new Set(navigationPaths)], testableFeatures, userFlows, summary }
  }
  ...
  private analyzeFromPayload(url: string, payload: unknown): UIAnalysisOutput {
    ...
    for (const n of nodes) {
      ...
      if (tag === 'button' || role === 'button') buttonElements.push({ text: n.text, id: n.id, classList: n.classList })
