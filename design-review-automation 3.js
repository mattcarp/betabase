    ...
    try {
        ...
        const gamingElementsLogin = await page.evaluate(() => {
            ...
            if (glowElements.length > 0) {
                elements.push(`Found ${glowElements.length} glow/shimmer effects - GAMING AESTHETIC DETECTED`);
            }
        ...
        for (let route of routesToTest) {
            try {
                ...
                const gamingElements = await page.evaluate(() => {
                    ...
                    const glowElements = document.querySelectorAll('[class*="glow"], [class*="shimmer"], [class*="pulse"]');
                    if (glowElements.length > 0) {
            ...
            allElements.forEach(el => {
                ...
                if (typeof classes === 'string') {
                    ...
                    if (fontWeight && !['100', '200', '300', '400', 'normal', 'bold'].includes(fontWeight)) {
                        analysis.nonCompliantElements.push(`Heavy font weight (${fontWeight}) on ${el.tagName}`);
                    }
